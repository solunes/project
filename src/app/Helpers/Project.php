...
namespace Solunes\Project\App\Helpers;
...
class Project {
    ...
    public static function get_cart() {
        if($cart = \Solunes\Project\App\Cart::checkOwner()->checkCart()->status('holding')->with('cart_items','cart_items.product')->first()){
          $cart->touch();
        } else {
          $cart = new \Solunes\Project\App\Cart;
          if(\Auth::check()){
    ...
    public static function add_cart_item($cart, $product, $quantity) {
        if($cart_item = $cart->cart_items->where('product_id', $product->id)->first()){
          ...
          $cart_item = new \Solunes\Project\App\CartItem;
    ...
    public static function check_category_children($category, $category_array = []) {
        ...
        if(count($category->children)>0){
          foreach($category->children as $subcategory){
            $category_array = \Project::check_category_children($subcategory, $category_array);
          }
        }
    ...
    public static function generate_code($type) {
        if($last = \Solunes\Project\App\TransactionCode::orderBy('code', 'DESC')->first()){
            $last_code = $last->code;
        } else {
            $last_code = 0;
        }
        $new = $last_code + 1;
        $new_code = new \Solunes\Project\App\TransactionCode;
        $new_code->type = $type;
        $new_code->code = $new;
        $new_code->save();
        return $new;
    ...
    public static function register_sale_payment($sale, $payment_id, $currency_id, $status, $amount, $detail, $exchange = 1) {
        $sale_payment = new \Solunes\Project\App\SalePayment;
    ...
    public static function register_account_array($array, $created_at = NULL, $transaction_code = NULL) {
        if(!$transaction_code){
            $transaction_code = \Project::generate_code('auto');
        }
        if(!$created_at){
        ...
        foreach($array as $item){
            if($item['amount']>0){
                $account_detail = new \Solunes\Project\App\PlaceAccountability;
    ...
    public static function inventory_movement($place, $product, $type, $quantity, $name, $transaction, $item, $transaction_code = NULL) {
        ...
        $product_movement = new \Solunes\Project\App\InventoryMovement;
        ...
        if($product_stock = \Solunes\Project\App\ProductStock::where('parent_id', $product->id)->where('place_id', $place->id)->first()){
            $product_stock->quantity = $product_stock->quantity + $real_quantity;
        } else {
            $product_stock = new \Solunes\Project\App\ProductStock;
        ...
        if($transaction!='register_product_purchase'){
            ...
            $asset_stock = \Solunes\Project\App\Account::getCode('asset_stock')->id;
            if($transaction=='register_product_drop'){
                $expense_sale = \Solunes\Project\App\Account::getCode('expense_inventory_loss')->id;
            } else {
                $expense_sale = \Solunes\Project\App\Account::getCode('expense_sale')->id;
            }
            if($type=='move_out'){
                $stock_type = 'credit';
                $expense_type = 'debit';
            ...
            $arr[] = \Project::register_account($place->id, $stock_type, $asset_stock, $currency_id, $amount, $name);
            $arr[] = \Project::register_account($place->id, $expense_type, $expense_sale, $currency_id, $amount, $name);
            \Project::register_account_array($arr, $item->created_at, $transaction_code);
        }
    }
    ...
    public static function check_report_header($model, $places = [], $extra = NULL) {
        ...
        $currency = \Solunes\Project\App\Currency::find($currency_id);
        $currencies = \Solunes\Project\App\Currency::where('in_accounts', 1)->get()->lists('real_name', 'id');
        ...
        if($extra&&$extra=='account_id'){
            ...
            $array['accounts'] = \Solunes\Project\App\Account::lists('name','id');
            $array['current_account_id'] = 1;
            if(request()->has('account_id')){
                $array['current_account_id'] = request()->input('account_id');
            }
        }
        $array['places'] = ['all'=>'Consolidado']  + \Solunes\Project\App\Place::lists('name', 'id')->toArray() + $places;
    ...
    public static function calculate_shipping_cost($shipping_id, $city_id, $weight) {
        $shipping = \Solunes\Project\App\Shipping::find($shipping_id);
